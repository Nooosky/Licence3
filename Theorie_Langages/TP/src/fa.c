...
{
  if(-1 < alpha_count && alpha_count <= 26)
  {
    if(-1 < state_count)
    {
    ...
    {
      perror("fa_create, state_count");
      exit (1);
    }
  }
  else
  {
    perror("fa_create, alpha_count");
    exit (1);
  }
}
...
void fa_remove_transition(const struct fa *self, size_t from, char alpha, size_t to)
{
  if(-1 < (int)from && from < self->state_count)
  {
    if(-1 < (alpha - 'a') && (alpha - 'a') < self->alpha_count)
    {
      ...
      {
        perror("fa_remove_transition, to");
        exit (1);
      }
    }
    else
    {
      perror("fa_remove_transition, alpha");
      exit (1);
    }
  }
  else
  {
    perror("fa_remove_transition, from");
    exit (1);
  }
}
