...
TEST(fa_pretty_print, file_input)
{
  struct fa *selfFa = (struct fa *) malloc(sizeof(struct fa));
  fa_create(selfFa, 2, 3);

  std::string str = "txt/wrong_txt_file.txt";
  char * writable = new char[str.size() + 1];
  std::copy(str.begin(), str.end(), writable);
  writable[str.size()] = '\0'; // don't forget the terminating 0

  std::string str0 = "test_txt.txt";
  ...
  EXPECT_EQ(-1, fa_pretty_print(selfFa, file, writable));

  EXPECT_EQ(0, fa_pretty_print(selfFa, file, writable0));
